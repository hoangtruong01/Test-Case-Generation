...
const EndpointsPage = () => {
  ...
  // Auto-select collection when ?collectionId=... is present
  const location = useLocation();
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const cid = params.get("collectionId");
    if (!cid) return;
    if (collections.length === 0) return; // wait until collections loaded
    const match = collections.find((c) => String(c.id) === String(cid));
    if (match) {
      handleSelectCollection(match);
    }
  ...
  const handleSelectCollection = (collection: Collection) => {
    ...
    api
      .getPostmanRequests(collection.id)
      .then((res) => {
        if ("error" in res) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Select a collection to view its endpoints
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className={`px-2.5 py-0.5 text-xs font-bold rounded uppercase shrink-0 ${getMethodClass(req.request?.method)}`}
                      >
                        {req.request?.method || "???"}
                      </span>
                      ...
                      >
                        ...
                        <div className="pb-2">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <code className="text-sm font-mono bg-muted px-2 py-1 rounded flex-1 overflow-x-auto">
                              {urlStr}
                            </code>
                            <button
                        ...
                        {req.request?.body?.raw && (
                          <div className="pb-2">
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                onClick={() =>
                                  copyToClipboard(req.request?.body?.raw)
                                }
                            ...
                            <pre className="mt-1 p-3 rounded bg-muted text-xs font-mono overflow-auto max-h-48">
                              {req.request?.body?.raw}
                            </pre>
                          </div>
                        )}
